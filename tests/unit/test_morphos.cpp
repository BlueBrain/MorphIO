...
BOOST_AUTO_TEST_CASE( test_morpho_tree )
{
    ...
    BOOST_CHECK_EQUAL(first_node.is_of_type(morpho_node_type::neuron_soma_type), true);
    BOOST_CHECK_EQUAL(first_node.is_of_type(morpho_node_type::neuron_node_3d_type), true);
    BOOST_CHECK_EQUAL(first_node.is_of_type(morpho_node_type::neuron_branch_type), false);
    ...
    BOOST_CHECK(soma.get_branch_type() == neuron_struct_type::soma);


    BOOST_CHECK_EQUAL(tree.get_children(0).size(), 0);

    std::shared_ptr<morpho_node> node2(new neuron_branch(
    ...
    BOOST_CHECK_EQUAL(second_node.is_of_type(morpho_node_type::neuron_branch_type), true);
    BOOST_CHECK_EQUAL(second_node.is_of_type(morpho_node_type::neuron_node_3d_type), true);
    BOOST_CHECK_EQUAL(second_node.is_of_type(morpho_node_type::neuron_soma_type), false);

    const neuron_branch & branch1 = static_cast<const neuron_branch &>(second_node);
    BOOST_CHECK(branch1.get_branch_type() == neuron_struct_type::dentrite_basal);
...
BOOST_AUTO_TEST_CASE( test_load_h5v1 )
{
...

    {
         ...
         BOOST_CHECK_EQUAL(tree.get_node(0).is_of_type(morpho_node_type::neuron_soma_type), true);
         const neuron_soma & soma = static_cast<const neuron_soma &>(tree.get_node(0));
         BOOST_CHECK(soma.get_branch_type() == neuron_struct_type::soma);

         BOOST_CHECK_EQUAL(soma.get_line_loop().size(), reader.get_branch_range_raw(0).second);

         for(std::size_t i = 1 ; i < tree.get_tree_size(); ++i){
             BOOST_CHECK_EQUAL(tree.get_node(i).is_of_type(morpho_node_type::neuron_branch_type), true);

             const neuron_branch & nbranch = static_cast<const neuron_branch&>(tree.get_node(i));

             const std::size_t branch_raw_offset = reader.get_branch_range_raw(i).first;
             const std::size_t branch_raw_size = reader.get_branch_range_raw(i).second;
             BOOST_CHECK_EQUAL(nbranch.get_number_points(), branch_raw_size);
             BOOST_CHECK_EQUAL(nbranch.get_points().size(), branch_raw_size);
             BOOST_CHECK_EQUAL(nbranch.get_radius().size(), branch_raw_size);

             BOOST_CHECK(nbranch.get_branch_type() == neuron_struct_type::axon
                         || nbranch.get_branch_type() == neuron_struct_type::dentrite_basal
                         || nbranch.get_branch_type() == neuron_struct_type::dentrite_apical);



             for(std::size_t j = 0; j < nbranch.get_number_points(); ++j){
                 const auto & points = nbranch.get_points();
                 const auto & radius = nbranch.get_radius();

                 point point_raw(raw_points(branch_raw_offset+j, 0), raw_points(branch_raw_offset+j, 1), raw_points(branch_raw_offset+j, 2));
                 double radius_raw = raw_points(branch_raw_offset+j, 3) /2;
...
void for_each_point_branch(const morpho::morpho_node & node, Fun f){
    using namespace morpho;
...
void for_each_point_tree(const morpho::morpho_tree & tree, Fun f){
    ...
    for(std::size_t i = 0; i < tree.get_tree_size(); ++i){
        ...
        for_each_point_branch<Fun>(node, f);
    }
}
...
template<typename Fun>
void for_each_point_branch_tree(const morpho::morpho_tree & tree, Fun f){
    using namespace morpho;
    for(std::size_t i = 1; i < tree.get_tree_size(); ++i){
        const morpho_node & node = tree.get_node(i);
        for_each_point_branch<Fun>(node, f);
...
BOOST_AUTO_TEST_CASE( test_bounding_box )
{
    ...
    for(std::size_t i =0; i < tree.get_tree_size(); ++i){
        ...
        std::cout << "branch bounding box " << node_box.min_corner() <<  " " << node_box.max_corner() << std::endl;

        for_each_point_branch(node, [&](const point & p){
