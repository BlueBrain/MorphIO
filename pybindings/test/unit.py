...
class Test_H5V1(unittest.TestCase):
    ...
    def test_reader(self):
        ...
        self.assertTrue(soma.branch_type == NEURON_STRUCT_TYPE.soma)
        self.assertEqual(len(soma.line_loop), reader.get_branch_range_raw(0)[1])

        # We keep C++ style to avoid introducing mismatches to the original version
        for i in range(1, tree.tree_size):
            nbranch = tree.get_node(i)
            self.assertTrue(nbranch.is_of_type(MORPHO_NODE_TYPE.neuron_branch_type))
            branch_raw_offset = reader.get_branch_range_raw(i)[0]
            branch_raw_size = reader.get_branch_range_raw(i)[1]
            self.assertEqual(nbranch.number_points, branch_raw_size)
            self.assertEqual(len(nbranch.points), branch_raw_size)
            self.assertEqual(len(nbranch.radius), branch_raw_size)
            self.assertTrue(nbranch.branch_type in (NEURON_STRUCT_TYPE.axon,
                                                    NEURON_STRUCT_TYPE.dentrite_basal,
                                                    NEURON_STRUCT_TYPE.dentrite_apical))

            for j in range(nbranch.number_points):
                # Raw points map to Numpy structures -> cool for indexing and math
                point_raw = (raw_points[branch_raw_offset+j, 0:3])
                radius_raw = raw_points[branch_raw_offset+j, 3] / 2
