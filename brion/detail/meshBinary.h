...
template< typename T > T get( const uint8_t* buffer, size_t& pos )
{
...
public:
    ...
    {
        if( !_ptr )
            LBTHROW( std::runtime_error( "Could not open mesh file: " +
                                         source ));
    ...
    virtual void writeVertexSections( const uint16_ts& vSections )
    {
        if( _vertices != vSections.size( ))
            LBTHROW( std::runtime_error( "Number of vertices does not match "
    ...
    {
        LBTHROW( std::runtime_error( "No triangle sections support for binary "
                                     "mesh files" ));
    }

    virtual void writeTriangleDistances( const floats& /*tDistances*/ )
    {
        LBTHROW( std::runtime_error( "No triangle distances support for binary "
                                     "mesh files" ));
    }
    ...
    {
        LBTHROW( std::runtime_error( "No normal support for binary mesh "
                                     "files" ));
    }
    ...
    virtual void writeStructureVertices( const Vector3fs& /*vertices*/,
                                         ...
                                         const size_t /*index*/ )
    {
        LBTHROW( std::runtime_error( "No structural mesh support for binary "
                                     "mesh files" ));
    }
