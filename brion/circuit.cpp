...
public:
    explicit Impl(const std::string& source)
    {
        ...
        sections.insert(
            std::make_pair("Layers Positions Data", SECTION_LAYERS));
        sections.insert(
            std::make_pair("Axon-Dendrite Synapses", SECTION_SYNAPSES));
        sections.insert(std::make_pair("ElectroTypes", SECTION_ETYPES));
        sections.insert(std::make_pair("MorphTypes", SECTION_MTYPES));
        sections.insert(
            std::make_pair("MiniColumnsPosition", SECTION_MCPOSITIONS));
        sections.insert(std::make_pair("CircuitSeeds", SECTION_CIRCUITSEEDS));
