...
**/
void soma_swc_to_contour(morphio::mut::Morphology& morpho) {
    auto somaType = morpho.somaType();

    if(somaType == morphio::SOMA_NEUROMORPHO_THREE_POINT_CYLINDERS) {
        LBERROR("Transforming SWC soma format to a contour of equivalent surface");
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "morpho.soma()->surface(): " << morphology.soma()->surface() << std::endl;

    if(source_extension == "swc" &&
       (destination_extension == "asc" || destination_extension == "h5")) {
        soma_swc_to_contour(morphology);
    }
    ...
    else if(destination_extension == "h5") {
