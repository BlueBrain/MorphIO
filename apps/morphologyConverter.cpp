...
int main( int argc, char* argv[] )
{
    ...
    const std::string& input = vm.count( "input" ) == 1 ?
        vm["input"].as< std::string >() : std::string();
    const std::string& output = vm.count( "output" ) == 1 ?
        vm["output"].as< std::string >() : std::string();
    ...
    BOOST_FOREACH( const brion::MorphologyRepairStage stage, stages )
    {
        ...
        LBDEBUG << points->size() << " points, " << sections->size()
                << " sections in stage "
                << lexical_cast< std::string >( stage ) << std::endl;
    }
    writeTime += clock.resetTimef();
    ...
    LBINFO << "Converted " << input <<  " (" << in.getVersion() << ") => "
           ...
           << " + " << writeTime << " ms (" << stages.size() - emptyStages
           << " stages)" << std::endl;
    return EXIT_SUCCESS;
}
