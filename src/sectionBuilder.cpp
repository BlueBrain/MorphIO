...
uint32_t Morphology::appendSection(Section* parent, const minimorph::Section& section,
                                   bool recursive)
{
...
void Morphology::_register(Section* section)
{
    ...
    if (_sections[section->id()])
    {
        ...
        ss << "Cannot register section (" << section->id()
           << "). The morphology has already a section with the same ID." << std::endl;
        LBTHROW(ss.str());
    }
...
{
    _somaType = soma.type();
    _pointProperties = Property::PointLevel(soma._properties->_pointLevel, soma._range);
}
...
Section::Section(Morphology* morphology, const minimorph::Section& section, bool recursive)
    : Section(morphology, section.id(), section.type(),
              Property::PointLevel(section._properties->_pointLevel, section._range))
{
    if (recursive)
    {
...
void Morphology::traverse(std::function<void(Morphology* morphology, Section* section)> fun,
                          Section* rootSection)
{
...
Property::Properties Morphology::buildReadOnly()
{
    ...
    auto writeSection = [&start, &properties, &sectionIdOnDisk, &newIds](Morphology* morpho,
                                                                         Section* section) {
...
void swc(Morphology& morphology)
{
    ...
    auto writeSection = [&sectionIdOnDisk, &newIds](Morphology* morphology, Section* section) {
        ...
        {
            std::cout << sectionIdOnDisk << ' ' << section->type() << ' ' << points[i][0] << ' '
                      << points[i][1] << ' ' << points[i][2] << ' ' << diameters[i] / 2. << ' ';
            if (i > 0)
...
void _write_asc_points(const Points& points, const std::vector<float>& diameters, int indentLevel)
{
    for (int i = 0; i < points.size(); ++i)
    {
        std::cout << std::string(indentLevel, ' ') << "(" << points[i][0] << ' ' << points[i][1]
                  << ' ' << points[i][2] << ' ' << diameters[i] << ')' << std::endl;
    }
}
...
void h5(Morphology& morphology)
{
    ...
    auto writeSection = [&start, &properties, &sectionIdOnDisk, &newIds](Morphology* morpho,
                                                                         Section* section) {
        int parentOnDisk = (section->parent() ? newIds[section->parent()->id()] : 0);
        std::cout << setw(6) << sectionIdOnDisk << ' ' << setw(6) << parentOnDisk << ' ' << setw(6)
                  << start << ' ' << setw(6) << section->type() << std::endl;
        newIds[section->id()] = sectionIdOnDisk++;
        start += section->points().size();
    };
    ...
    std::cout << setw(6) << 0 << ' ' << setw(6) << -1 << ' ' << setw(6) << start << ' ' << setw(6)
              << SECTION_SOMA << std::endl;
