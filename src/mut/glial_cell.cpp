...
GlialCell::GlialCell(const std::string& uri, unsigned int options)
    : GlialCell(morphio::GlialCell(uri, options)) {}

GlialCell::GlialCell(const morphio::mut::GlialCell& morphology, unsigned int options)
    : _counter(0)
    ...
    for (const std::shared_ptr<GlialSection>& root : morphology.rootSections()) {
...
GlialCell::GlialCell(const morphio::GlialCell& morphology, unsigned int options)
    : _counter(0)
    , _soma(std::make_shared<Soma>(morphology.soma()))
    , _endoplasmicReticulum(morphology.endoplasmicReticulum()) {
...
uint32_t GlialCell::_register(const std::shared_ptr<GlialSection>& section_) {
    if (_sections.count(section_->id()))
        throw SectionBuilderError("GlialSection already exists");
...
static void eraseByValue(std::vector<std::shared_ptr<GlialSection>>& vec,
                         const std::shared_ptr<GlialSection>& section) {
    vec.erase(std::remove(vec.begin(), vec.end(), section), vec.end());
}

void GlialCell::deleteSection(const std::shared_ptr<GlialSection>& section_, bool recursive)
...
Property::Properties GlialCell::buildReadOnly() const {
    using std::setw;
...
std::unordered_map<int, std::vector<unsigned int>> GlialCell::connectivity() {
    ...
    std::transform(roots.begin(),
                   ...
                   [](const std::shared_ptr<GlialSection>& section) { return section->id(); });
...
void GlialCell::write(const std::string& filename) {
    const size_t pos = filename.find_last_of(".");
    assert(pos != std::string::npos);

    std::string extension;

    morphio::mut::GlialCell clean(*this);
    clean.sanitize();
