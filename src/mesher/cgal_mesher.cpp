...
static sphere bounding_box_to_bounding_sphere(const box& g_box) {
    ...
    double max_distance =
        hg::distance(g_box.min_corner(), g_box.max_corner()) / 2.0;
...
FT is_part_of_morphotree(const Point& p, const spatial_index& index,
    ...
    if (stats.total_iterations % 100000 == 0) {
        ...
        fmt::scat(std::cout, "Geometry scan progress: ", stats.total_iterations,
                  " points check with ", stats.within_iterations,
                  " positives matches", end_line);
        std::flush(std::cout);
    }
...
morpho_mesher::morpho_mesher(const std::shared_ptr<morpho_tree>& tree,
                             const std::string& output_mesh_file)
    : _flags(), _error_bound(1.0 / 100000), _facet_size(0.3),
      _output_mesh_file(output_mesh_file), _tree(tree) {}

void morpho_mesher::set_mesh_tag(mesh_tag tag, bool value) {
    _flags[int(tag)] = value;
}

void morpho_mesher::set_error_bound(double v) { _error_bound = 1.0 / v; }
...
void morpho_mesher::log_parameters() {
    ...
    std::cout << "-- optimisation of mesh "
              << ((_flags[mesh_optimisation]) ? ("ENABLED") : ("DISABLED"))
              << "\n";
}
...
void morpho_mesher::execute_3d_meshing() {
    ...
    sphere bounding_sphere =
        bounding_box_to_bounding_sphere(_tree->get_bounding_box());

    mesh_logger(fmt::scat("Configure bounding sphere with radius of ",
                          bounding_sphere.get_radius()));
    ...
    Mesh_domain domain(
        domain_distance,
        K::Sphere_3(hadoken_to_cgal_point(bounding_sphere.get_center()),
                    bounding_sphere.get_radius() *
                        bounding_sphere.get_radius()),
        _error_bound);
    // Mesh criteria
    // Spherical_sizing_field size;
    Mesh_criteria criteria(facet_angle = 30, facet_size = _facet_size,
                           facet_distance = _facet_size / 4,
                           cell_radius_edge_ratio = 2,
                           cell_size = _facet_size /*0.1*8*/);
    ...
    if (_flags[mesh_optimisation]) {
        ...
        c3t3 = CGAL::make_mesh_3<C3t3>(
            domain, criteria, CGAL::parameters::no_lloyd(),
            CGAL::parameters::no_odt(), CGAL::parameters::no_perturb(),
    ...
    fmt::scat(std::cout, "\n");
    mesh_logger(fmt::scat(
        "End mesh generation in ",
        std::chrono::duration_cast<std::chrono::seconds>(t2 - t1).count(),
        "s"));
    mesh_logger(
        fmt::scat("Meshing domain scan done with ", stats.total_iterations,
                  " points checks and ",
                  double(stats.within_iterations) / stats.total_iterations,
                  " of positive matches "));

    // Output
    {
        const std::string local_3dmesh_output_file =
            _output_mesh_file + ".3d.mesh";
    ...
    {
        const std::string local_surface_mesh_output_file =
            _output_mesh_file + ".surface.off";
        fmt::scat(std::cout, "- output surface mesh to ",
                  local_surface_mesh_output_file, "\n");
...
void morpho_mesher::execute_surface_meshing() {
    ...
    CGAL::Surface_mesh_default_criteria_3<Tr_2D> criteria(
        30.,          // angular bound
