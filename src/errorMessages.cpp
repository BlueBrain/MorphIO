...
std::string ErrorMessages::ERROR_PARSING_POINT(long unsigned int lineNumber,
    ...
    return errorMsg(lineNumber, ErrorLevel::ERROR, "Error converting: \"" + point + "\" to float");
}
...
std::string ErrorMessages::WARNING_NEUROMORPHO_SOMA_NON_CONFORM(const Sample& root,
                                                                const Sample& child1,
                                                                const Sample& child2) {
    float x = root.point[0], y = root.point[1], z = root.point[2], r = root.diameter / 2.f;
    std::stringstream ss;
    ss << "Warning: the soma does not conform the three point soma spec\n"
       ...
       << _col(child1.point[2], z) << ' ' << _col(child1.diameter / 2.f, r)
       << " 1\n"
          "3 1 "
       << _col(child2.point[0], x) << ' ' << _col(child2.point[1], y + r) << ' '
       << _col(child2.point[2], z) << ' ' << _col(child2.diameter / 2.f, r) << " 1\n";
