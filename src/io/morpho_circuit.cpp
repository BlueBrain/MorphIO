                    ...
                    if (exists(morphology_path)) {
                        try {
...

                            {
                                ...
                                circuit_morpho_tree.emplace_back(
                                    std::move(morpho_transposed));
                            }

                        } catch (std::exception& e) {
                            throw std::runtime_error(hadoken::format::scat(
                                "Impossible to open morphology ",
                                morphology_path, " in circuit ", _filename,
                                "\n", e.what()));
                        }
                    }
                }
