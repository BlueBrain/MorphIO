...
template <> bool compare(const morphio::range<const morphio::Point>& vec1,
...
bool compare(const PointLevel& el1, const PointLevel& el2,
             size_t soma_offset1, size_t soma_offset2,
             const std::string& name, bool verbose) {
    if(&el1 == &el2)
        return true;
    ...
    morphio::range<const typename Diameter::Type> diameters1(
        el1._diameters.data() + soma_offset1, el1._diameters.size() - soma_offset1);
    morphio::range<const typename Diameter::Type> diameters2(
        el2._diameters.data() + soma_offset2, el2._diameters.size() - soma_offset2);
    ...
    if (el1._perimeters.size() > soma_offset1 &&
        ...
        morphio::range<const typename Perimeter::Type> perimeters1(
            el1._perimeters.data() + soma_offset1, el1._perimeters.size() - soma_offset1);
        morphio::range<const typename Perimeter::Type> perimeters2(
            el2._perimeters.data() + soma_offset2, el2._perimeters.size() - soma_offset2);
...
bool Properties::operator==(const Properties& other) const {
    ...
    return (compare(this -> _pointLevel,
                    other._pointLevel,
                    this_soma_offset,
                    other_soma_offset,
                    "_pointLevel",
